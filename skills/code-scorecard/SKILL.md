...
For .NET 2.3.0+, failed dependency assessment is a tool/evidence limitation, not a code defect. Read `dependencyCompatibility.status`, `failures`, unique-package and observation counts, and `vulnerabilityAssessmentAvailable`. Explain source-discovery, download, budget, unavailable-version or unsupported-metadata reasons as recorded; do not guess an outage cause from an unknown count. An `outdatedDependency` observation with `scoreDisposition: unavailable` is not a verified upgrade issue or penalty. Preserve confirmed vulnerability/deprecation findings from independently successful checks, but do not infer a failed dimension's score from them. A failed vulnerability check also withholds Security's score while preserving static findings. Failed assessment means no overall score; never substitute zero, ten, cached results or CSV. Group failures by package/version and distinguish their affected project/TFM counts from unique lookups. Retrying after fixing the assessment problem is an appropriate priority.
