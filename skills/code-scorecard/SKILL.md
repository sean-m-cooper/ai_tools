...
7. Attribute a score to specific triggering conditions only when the recorded scoring explanation establishes them or you have verified the policy for the reported analyzer version. Finding counts and aggregate score loss alone do not establish which condition selected a score. For ladders and caps, distinguish the active condition from other findings that did not further reduce the score. When attribution is unavailable, report the score and observations separately without inventing combined deductions. Keep policy calculations in the analyzer.
