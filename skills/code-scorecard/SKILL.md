...
Display the .NET `codeQuality` dimension as **Complexity & Decomposition**. Keep its recorded combined score as one of the nine dimensions. Show **Method complexity** and **Decomposition** scores in that row's evidence cell and explain them separately below the table, with separately ranked type hotspots from `componentDetails` when available. Read scores from the corresponding `scoringDecision.steps` (`complexity`, `decomposition`), or the recorded `metrics.maxMemberCyclomaticComplexity.ccScore` and `metrics.decomposition.decompScore` in older evidence. Missing component values are unavailable, never inferred from the combined score. A zero eligible population is unmeasured; disclose any policy default rather than implying measured excellence.

Method complexity uses each eligible type's maximum member CC, not the proportion of all methods. Decomposition uses class CC divided by member count for types with at least two members; it does not establish cohesion, readability or correctness. Trivial helper extraction can improve the ratio without improving the code. Preserve project/TFM identity and denominators when presenting hotspots. Older `topOffenders` is ranked by decomposition and must not be presented as the worst method-complexity list. The components remain within one dimension: do not count either again in the overall or change the denominator. For JS/TS, label its current `codeQuality` signal **Method Complexity** and report decomposition as unmeasured, since its policy does not implement that component.
