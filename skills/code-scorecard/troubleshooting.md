...
- **New analyzer, old reporting behavior:** analyzer packages and skill instructions update independently. Check the actual `SKILL.md` loaded for the invocation, including its resolved path, against the intended ai_tools revision. A current tool version does not establish current skill instructions. Report a mismatch and have the active skill installation updated through its normal distribution mechanism; updating the analyzer again will not refresh the skill. Do not rewrite cached instructions or repository pins as a workaround.
